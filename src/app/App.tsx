...
// ============================================================
// KONSTANTA INTERNAL — pilihan konfirmasi dari bot
// ============================================================
const OPT_LANJUT   = "Ya, saya masih ingin bertanya";
const OPT_SELESAI  = "Tidak, terima kasih";
        ...
        {!showForm ? (
          <>
            ...
            <h3 className="font-bold text-gray-800 text-lg mb-2">
              Terima kasih telah menggunakan SemaBot!
            </h3>
            <p className="text-gray-500 text-sm mb-6">
              Mohon luangkan waktu untuk mengisi survei kepuasan pengguna kami.
            </p>
    ...
    return (
      <div className="flex items-end gap-2 px-4">
        ...
        <div className="flex flex-col gap-0.5 max-w-[72%]">
          ...
          {/* Tombol pilihan (klarifikasi topik ATAU konfirmasi lanjut/selesai) */}
          {message.options && message.options.length > 0 && (
            <div className="flex flex-col gap-1.5 mt-1.5">
              {message.options.map((opt) => {
                // Styling khusus untuk tombol konfirmasi selesai
                const isSelesai = opt === OPT_SELESAI;
                return (
                  <button
                    key={opt}
  ...
  return (
    <div className="flex items-end gap-2 px-4 justify-end">
      <div className="flex flex-col gap-0.5 max-w-[72%] items-end">
        <div
          className="px-4 py-2.5 text-[0.875rem] leading-relaxed rounded-2xl rounded-br-sm text-white"
          style={{
...
export default function App() {
  ...
  // Lacak apakah pesan konfirmasi sudah pernah dikirim di sesi ini
  // agar tidak muncul dua kali berturut-turut
  ...
  const sendMessage = async (text: string) => {
    ...
    // Reset flag agar konfirmasi bisa muncul lagi setelah interaksi baru
    ...
    try {
      ...
      // ✅ Tentukan apakah perlu mengirim pesan konfirmasi lanjut/selesai:
      // — Tidak jika jawaban adalah fallback (topik tidak dikenal / di luar layanan)
      // — Tidak jika jawaban masih menunggu klarifikasi topik (options dari bot)
      // — Tidak jika konfirmasi sudah dikirim di giliran sebelumnya
      const isFallback =
        replyData.reply.includes("di luar layanan") ||
        replyData.reply.includes("di luar kewenangan") ||
        replyData.reply.includes("belum bisa menjawab");
      const isAmbiguous = replyData.options && replyData.options.length > 0;

      if (!isFallback && !isAmbiguous && !confirmSentRef.current) {
        confirmSentRef.current = true;
        // Delay 1.5 detik agar user sempat mulai membaca jawaban
        setTimeout(() => {
          setMessages((prev) => [
            ...prev,
            {
              id: Date.now() + 2,
              from: "bot",
              text: "Apakah ada yang ingin Anda tanyakan lagi? 😊",
              time: getTime(),
              options: [OPT_LANJUT, OPT_SELESAI],
            },
          ]);
        }, 1500);
      }
  ...
  // ── Tangani klik tombol pilihan ────────────────────────────
  const handleSelectOption = (optionText: string) => {
    // Pilihan: Tidak, terima kasih → tampilkan popup survei
    if (optionText === OPT_SELESAI) {
      const userMsg: Message = {
        id: Date.now(),
        from: "user",
        text: optionText,
      ...
      const botMsg: Message = {
        id: Date.now() + 1,
        from: "bot",
        text: "Terima kasih telah menggunakan layanan SemaBot Disdukcapil Kota Semarang! 🙏<br>Jika berkenan, mohon isi survei kepuasan kami untuk membantu kami meningkatkan layanan.",
        time: getTime(),
      };
      setMessages((prev) => [...prev, userMsg, botMsg]);
      // Delay singkat sebelum popup muncul agar pesan bot terbaca dulu
      setTimeout(() => setShowSurvey(true), 900);
      return;
    }

    // Pilihan: Ya, saya masih ingin bertanya → beri respons singkat & reset flag
    if (optionText === OPT_LANJUT) {
      confirmSentRef.current = false;
      const userMsg: Message = {
        id: Date.now(),
        from: "user",
      ...
      const botMsg: Message = {
        id: Date.now() + 1,
        from: "bot",
        text: "Baik! Silakan ketik pertanyaan Anda berikutnya 😊",
        time: getTime(),
      };
      setMessages((prev) => [...prev, userMsg, botMsg]);
    ...
    // Pilihan klarifikasi topik (akta, ubah KK, dll.) → kirim sebagai pesan baru
  ...
  return (
    <div
      ...
      style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: "#f5e0e0" }}
    >
      {/* ── Survey Popup ── */}
      ...
      >
        ...
        {/* ── Header ── */}
        <header
          className="flex items-center gap-3 px-4 py-3 shadow-lg z-10"
          style={{
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-white font-bold text-[0.95rem] leading-tight truncate">
              SemaBot Disdukcapil
            </h1>
            <p className="text-red-200 text-[0.72rem] leading-tight truncate mt-0.5">
              Asisten Virtual Layanan Dukcapil Kota Semarang
            </p>
        ...
        <div className="flex items-center gap-3 px-6 pt-4 pb-1">
          ...
          <span
            className="text-[0.68rem] px-3 py-0.5 rounded-full font-medium"
        ...
        <div
          ...
          style={{
            backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23b91c1c' fill-opacity='0.03'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`,
          }}
        >
          {messages.map((msg) => (
            <ChatBubble
        ...
        {showScrollBtn && (
          <button
            ...
            className="absolute bottom-32 right-4 w-9 h-9 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-105 z-20"
            style={{
        ...
        <div className="px-4 pb-2 flex gap-2 overflow-x-auto">
          {quickReplies.map((q) => (
            <button
              ...
              disabled={typing}
              className="shrink-0 text-[0.75rem] px-3 py-1.5 rounded-full font-medium border transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50"
              style={{
                borderColor: "rgba(185,28,28,0.3)",
                color: "#b91c1c",
                background: "rgba(185,28,28,0.06)",
        ...
        >
          <div
            className="flex items-end gap-2 px-4 py-2 rounded-2xl shadow-sm"
            style={{
              background: "#fff",
